...
public class Game {
    ...
    private String secretChosenWord; // will store the word of each time that need to be guessed
    private StringBuilder underscores; // will store the needed unerscores for each secret word 
    ...
    public Game() {
        ...
        secretChosenWord = word.getWord(); // use the public method getWord() to access the random word and store it in the variable

        Display display = new Display(secretChosenWord); // creation of a new object of the class Display and giving the parameter (chosenWord)
        underscores = display.getLine(); // attrbiutes the Display's getLine() return to a variable

        verifier = new WordVerification(secretChosenWord); // creation of the object 
    ...
    private void startGame(){
        while (true) { // while the game is still up to play: 
            System.out.println(underscores.toString()); // print the array in String way
            ...
            if (correctGuess){ // if the wordVerification class returns true:
                for (int i = 0; i < secretChosenWord.length(); i++){ // according to the word's length
                    if (secretChosenWord.charAt(i) == guessedLetter){ // if the user's char is equal to any char of the String, the underscore will be replaced by the matching letter
                        underscores.setCharAt(i * 2, guessedLetter); // since the display indices are even for spaces and odd for characters, i must be multiplied by 2
                    }
                }
            ...
            if (underscores.indexOf("_ ") == -1){ // if the method does not find any underscore, it will return -1 (the word was fully guessed)
                System.out.println("You won! The guessed word was: " + secretChosenWord);
